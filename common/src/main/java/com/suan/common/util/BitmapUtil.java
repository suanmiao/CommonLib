...
import com.suan.common.util.helper.SystemHelper;
...
public class BitmapUtil {
    ...
    public static Bitmap decodePhoto(InputStream in, Photo photo) throws IOException {
        if (photo.getContentLength() > 0) {
            ...
            int sampleSize = getBestFitSampleSize(sourceWidth, sourceHeight, photo.getViewWidth(), photo.getViewHeight(), Bitmap.Config.ARGB_8888);
...
//    }
    ...
    public static byte[] streamToByteArray(InputStream in, Photo photo) throws IOException {
        ...
        while ((nRead = in.read(buffer, 0, buffer.length)) != -1) {
            ...
            readSize += nRead;
            if (photo.getProgressListener() != null && photo.getContentLength() > 0) {
                photo.getProgressListener().onProgress((readSize / photo.getContentLength() * READING_TAKE_UP_PERCENT));
            }
        }
        outputStream.flush();
    ...
    public static int getBestFitSampleSize(int sourceWidth, int sourceHeight, int targetWidth, int targetHeight, Bitmap.Config config) {
        /**
         * step:
...
//        int sampleSize = getBestFitSampleSize(sourceWidth, sourceHeight, screenSize[0], screenSize[1], Bitmap.Config.ARGB_8888);
//        options.inJustDecodeBounds = false;
//        options.inSampleSize = sampleSize;
